...
SOCKET_ERRORS_TO_RETRY = [9, 32, 104, 109, 111, 10061]
...
# Ten teraz edytuję
class OutputStreamClientConnector(BaseConnector):
    ...
    def __init__(self, service_instance):
        super(OutputStreamClientConnector,self).__init__(service_instance)
    ...
    def prepare_client_socket(self):
        if self.client_socket is None:
            ...
            self.client_socket.connect((self.get_output_ip(), self.get_output_port()))
...
class OutputMessageClientConnector(OutputStreamClientConnector):
    def send(self, message):
        message_length = len(message)
        super(OutputMessageClientConnector, self).send(struct.pack(MSG_LENGTH_FORMAT, message_length))
        super(OutputMessageClientConnector, self).send(message)

class OutputObjectClientConnector(OutputMessageClientConnector):
    def send(self, obj):
        dumped_object = json.dumps(obj)
        super(OutputObjectClientConnector, self).send(dumped_object)



class InputStreamClientConnector(BaseConnector):
    READ_TRY_INTERVAL = 3
    MAX_READ_RETRIES = 10
    SOCKET_TIMEOUT = 20

    def __init__(self, service_instance):
        super(InputStreamClientConnector,self).__init__(service_instance)
        self.client_socket = None
        self._params = {}
    ...
    def prepare_client_socket(self):
        if self.client_socket is None:
            self.client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.client_socket.connect((self.get_input_ip(), self.get_input_port()))
...
class InputMessageClientConnector(InputStreamClientConnector):

    def read_message(self):
        super(InputMessageClientConnector, self).prepare_client_socket()
    ...
    def read(self):
        counter = super(InputMessageClientConnector, self).MAX_READ_RETRIES
        while True:
            try:
                ...
                if __errno is  None:
                    ...
                    raise e
                counter -= 1
                super(InputMessageClientConnector, self).clear_client_socket_connection()
            else:
                if not read_buffer:
                    counter -= 1
                    super(InputMessageClientConnector, self).clear_client_socket_connection()
...
class InputObjectClientConnector(InputMessageClientConnector):
    def read(self):
        msg = super(InputObjectClientConnector, self).read()
